...
4. Con **Ver juntas 1** ves la gráfica de un solo fondo (selecciónalo en la lista de la izquierda), con la subida/bajada de su último valor liquidativo y el botón **Mis aportaciones**. En **Vista global** ves todos los fondos superpuestos, cada uno con su color; la leyenda va en su propio recuadro junto al resumen de tu cartera (con la rentabilidad de cada fondo en el periodo elegido y barra de desplazamiento si no caben todos), y pulsando un fondo se abre su gráfica. Al pasar el ratón por una gráfica sale una raya vertical y un bocadillo con la fecha y el valor de ese punto.
...
7. **Se actualiza sola**, sin tocar nada: al abrirla, al cambiar de usuario y cada 15 minutos mientras está en marcha, también escondida junto al reloj (editable en `misfondos/config.py`, variable `REFRESH_INTERVAL_SECONDS`). Un valor liquidativo recién publicado aparece como mucho 15 minutos después. Bajo los botones de la izquierda se ve la última actualización, el último valor liquidativo nuevo que ha llegado y la hora de la próxima comprobación. Para forzarla en algún caso raro: clic derecho en el icono del reloj → **Actualizar ahora**. **Eliminar seleccionado** pide confirmación antes de quitar un fondo.
...
10. **Riesgo**: pestaña con una tabla de todos los fondos calculada sobre el periodo elegido en la barra de meses: volatilidad anual, nivel de riesgo aproximado del 1 al 7 (tramos de la escala SRRI de los folletos), máxima caída desde un máximo y cuánto tardó en recuperarse, cuánto está ahora por debajo de su máximo y peor día. Cada cabecera explica su métrica al pasar el ratón; se ordena pulsándola (primero, el de menos riesgo).
